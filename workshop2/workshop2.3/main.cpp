...
void initBalls(std::vector<ball> &balls, unsigned BALL_SIZE, int MAX_SPEED)
{
    ...
    {
        balls[i].speed = {float(-MAX_SPEED + std::rand() % (2 * MAX_SPEED)), float(-MAX_SPEED + std::rand() % (2 * MAX_SPEED))};
        // balls[i].speed = {1.f + std::rand() % 100, 1.f + std::rand() % 100};
        std::cout << balls[i].speed.x << "  " << balls[i].speed.y << std::endl;
    }
}
...
int main()
{
    ...
    initBalls(balls, BALL_SIZE, MAX_SPEED);
    while (window.isOpen())
    {
